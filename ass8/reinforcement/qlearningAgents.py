...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        if len(action_list)==0:
          return 0
        Q_best = self.getQValue(state,action_list[0])
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        f_ext = self.featExtractor.getFeatures(state,action)
        for k in self.featExtractor.getFeatures(state,action).keys():
            f_ext[k] *= self.alpha*(reward+self.discount*self.computeValueFromQValues(nextState)- self.getQValue(state,action))
        self.weights = self.weights+ f_ext
        # util.raiseNotDefined()
