...
#function to print training accuracy
def trainingAccuracy(header,dataset):
    best_col = maxGainAttribute(dataset)
    tree = ID3DecisionTree(header,dataset)
    Nr = 0

    for r in dataset:
        if type(tree[header[best_col]]) is dict:
            d = tree[header[best_col]]
            col = best_col

            while type(d) is dict:
                if type(d[r[col]]) is not dict:
                    break
                d = d[r[col]]
                col = header.index(d.keys()[0])
                d = d[header[col]]
            # print(d)
            # print(r[col])
            # print(r[-1])
            # print(d[r[col]])
            if d[r[col]] == r[-1]: #in our dataset label is always last column
                Nr+=1
                # print(Nr)
    return (float(Nr)/float(len(dataset)))*100
...
if __name__ == "__main__":
    ...
    accuracy = trainingAccuracy(header,data)
    print("The training accuracy of the decision tree is "+str(accuracy))